 ...
 * @subpackage Admin
 * @todo 
 * @version $Id: downloads.php 214 2009-04-18 18:30:05Z JanMalte $
 * 
 * TODO check and improve security
 * TODO optomize functions
 * TODO clarify variable and function names
 * TODO comment code better
 * DONE don't add samples if there's already stuff there
 * DONE Erlauben von Unter-Kategorien (mehrere Ebenen)
 * DONE Ermöglichen des Uploads von Dateien im ACP (Daten sollen per FTP auf einem anderen Server landen)
 * TODO Portal-Box
 * DONE add settings for user uploads
 */

// Don't allow inc/plugins/downloads.php to be accesed directly.
if(!defined("IN_MYBB"))
{
...
function downloads_info()
{
	return array(
		...
		"description"	=> "A plugin that allows you to manage and display a list of downloadable files, each with it's own project page.",
		"website"		=> "http://www.mybbcoder.info/",
		"author"		=> "Jan",
		"authorsite"	=> "http://mybb.malte-gerth.de/",
		"version"		=> "214 2009-04-18 08:00:05Z",
		'compatibility' => '14*',
		'guid'		 => '',
	);
}
...
{
	// Require this to let us add a new template
	require "../inc/adminfunctions_templates.php";
	
	// Database access is nice
	global $db;
	
	// Add default templates
	downloads_addTemplates();
	...
	// Add Settings
	// Get the number of existing setting groups
	$query = $db->simple_select("settinggroups", "COUNT(*) as rows");
	$rows = $db->fetch_field($query, "rows");
	
	$dlsettinggroup = array(
		// The group name for database access
		'name'		=> 'dlmngr',
		// The group name for display
		'title'		=> 'Downloads Manager',
		'description'	=> 'These settings let you configure the Downloads Manager plugin.',
		// Make it the 'last' settings group
		'disporder'	=> $rows+1,
		'isdefault'	=> 0,
	);
	// Add it to the config menu, & save the groupID
	...
	$setting_5 = array(
		...
		// (*) Yes | ( ) No
		"optionscode"    => "yesno",
		// yes = 1, no = 0. Set default to 'yes' here.
		"value"          => '1',
		// The first setting in the group
		"disporder"      => '5',
		// add it to the group I just created
		"gid"            => intval($gid),
	);
	// store the setting
	...
	$setting_6 = array(
		...
		// ( ) Yes | (*) No
		"optionscode"    => "yesno",
		// yes = 1, no = 0. Set default to 'no' here.
		"value"          => '0',
		"disporder"      => '6',
		// add it to the group I just created
		"gid"            => intval($gid),
	...
	downloads_addSamples();
}

// This function tells MyBB whether or not the plugin is installed.
function downloads_is_installed()
{
	global $db;
	if($db->table_exists("downloads"))
 	{	// the installation script has been run, so it's installed (supposedly)
 		return true;
	}
	return false;
}
...
// Update the tables
function downloads_updateTables()
{
	global $db;
...
function downloads_addTemplates()
{
	global $db;
